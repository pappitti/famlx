...
from .configuration_utils import (
    PretrainedConfig, 
    _LazyAutoMapping, ### transferred from configuration_auto
    CONFIG_MAPPING_NAMES, ### transferred from configuration_auto
    AutoConfig, ### transferred from configuration_auto
    model_type_to_module_name, ### transferred from configuration_auto
    replace_list_option_in_docstrings ### transferred from configuration_auto
)
#from ...dynamic_module_utils import get_class_from_dynamic_module, resolve_trust_remote_code ### trust remote code should not be needed in this project
#from ...modeling_gguf_pytorch_utils import load_gguf_checkpoint ### gguf may not be needed
...
from .utils import (
    ...
    is_tokenizers_available, ### all relevant [fast] tokenizers have been commented out
    get_logger,
)

#from ..encoder_decoder import EncoderDecoderConfig
# from .auto_factory import _LazyAutoMapping ### class in configuration_utils now
# from .configuration_auto import ( ### classes and functions in configuration_utils now
#     CONFIG_MAPPING_NAMES,
#     AutoConfig,
#     # config_class_to_model_type,
#     model_type_to_module_name, ### may have to change this
#     replace_list_option_in_docstrings,
# )

if is_tokenizers_available():
    from .tokenization_utils import PreTrainedTokenizerFast 
else:
...
def tokenizer_class_from_name(class_name: str):
    ...
    for module_name, tokenizers in TOKENIZER_MAPPING_NAMES.items():
        if class_name in tokenizers:
            ...
            module = importlib.import_module(f".{module_name}", "tokenization.models")
            try:
...
class AutoTokenizer:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if config_tokenizer_class is None:
            if not isinstance(config, PretrainedConfig):
                ...
                if gguf_file:
                    ...
                    raise ValueError(
                        "You are trying to load a tokenizer from a model configuration file but provided a "
                        "`gguf_file`. This is not supported."
                    )
        ...
        ### should not happen through the MLX code in this project given the tokenizers we use
        ...
        # Otherwise we have to be creative. ### let's not be too creative in this project
