...
### from .dynamic_module_utils import custom_object_save #move to utils
from .utils import (
    ExplicitEnum,
    PaddingStrategy,
    PushToHubMixin,
    ...
    ### requires_backends,
    to_py_obj,
    custom_object_save, ### moved from .dynamic_module_utils
    _compile_jinja_template, ### moved from .utils.chat_template_utils
    _render_with_assistant_indices, ### moved from .utils.chat_template_utils
    PROTOBUF_IMPORT_ERROR,
    __version__
)
### from .utils.chat_template_utils import _compile_jinja_template, _render_with_assistant_indices ### moved to utils
# from .import_utils import (
#     PROTOBUF_IMPORT_ERROR, ### moved to utils
# )

### removed as it will be all handled as numpy arrays and converted to tensors upon generation (prompt_tokens = mx.array(tokenizer.encode(prompt)) in utils.py)
# if TYPE_CHECKING:
...
AudioInput = Union["np.ndarray", List["np.ndarray"]] ### removed torch as it should not be needed in Union["np.ndarray", "torch.Tensor", List["np.ndarray"], List["torch.Tensor"]]
...
class BatchEncoding(UserDict):
    ...
    ### tentatively commenting out as torch should not be needed
...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if len(unresolved_files) > 0: ### where is this even updated?
            logger.info(
    ...
    def _from_pretrained(
        ...
        **kwargs,
        ...
        ### given comment below, removing this block (should be safe and reduce dependencies)
    ...
    def pad(
        ...
        if not isinstance(first_element, (int, list, tuple)):
            ### removed as it will be np.ndarray
    ...
    def batch_decode(
        self,
        sequences: Union[List[int], List[List[int]], "np.ndarray"],  ### removed tf and torch from Union[List[int], List[List[int]], "np.ndarray", "torch.Tensor", "tf.Tensor"]
    ...
    def decode(
        self,
        token_ids: Union[int, List[int], "np.ndarray"], ### removed tf and torch from Union[int, List[int], np.ndarray, torch.Tensor, tf.Tensor]
        skip_special_tokens: bool = False,
        clean_up_tokenization_spaces: bool = None,
        **kwargs,
    ) -> str:
    ...
    ### tentatively removed the following method because it should not be used for this project
    # @classmethod
...
### removing the following method because it should not be used for this project
